      ...
      <nav class="main-menu d-flex navbar navbar-expand-lg ">
        ...
        <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
          ...
          <div class="offcanvas-body justify-content-between">
            ...
            <ul class="navbar-nav menu-list list-unstyled d-flex gap-md-3 mb-0">
                <li class="nav-item">
                    <a href="{{ route('index') }}" class="nav-link active header-text">Home</a>
                </li>
                @if (Auth::check())
                    <li class="nav-item">
                        <form action="{{ route('logout') }}" method="POST" class="d-inline">
                            @csrf
                            <button type="submit" class="nav-link btn btn-link header-text">Logout</button>
                        </form>
                    </li>
                    @if (Auth::user()->role == 2)
                        <li class="nav-item">
...
<div class="offcanvas offcanvas-end" data-bs-scroll="true" tabindex="-1" id="offcanvasCart" aria-labelledby="My Cart">
        ...
        <div class="offcanvas-body">
            <div class="order-md-last">
                ...
                <a href="#" class="w-100 btn btn-primary btn-lg">Continue to checkout</a>
            </div>
        </div>
    </div>
...
<div class="offcanvas offcanvas-end" data-bs-scroll="true" tabindex="-1" id="offcanvasCart" aria-labelledby="My Cart">
    ...
    <div class="offcanvas-body">
        <div class="order-md-last">
            ...
            <ul class="list-group mb-3">
                ...
                <li class="list-group-item d-flex justify-content-between lh-sm">
                    <div>
                        ...
                        <form action="{{ route('removeFromCart', $item->artwork->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            <button type="submit" class="btn btn-danger btn-sm">Remove</button>
                        </form>
                    </div>
                    <span class="text-body-secondary">₱{{ number_format($item->artwork->price * $item->quantity, 2) }}</span>
                </li>
                @endforeach
                <li class="list-group-item d-flex justify-content-between">
                    <span class="fw-bold">Total</span>
            ...
            <a href="{{ route('showCart') }}" class="w-100 btn btn-primary btn-lg">Continue to checkout</a>
        </div>
    </div>
</div>
          ...
          <div class="offcanvas-body justify-content-between">
            ...
            <ul class="navbar-nav menu-list list-unstyled d-flex gap-md-3 mb-0">
                ...
                @if (Auth::check())
                    ...
                    @elseif (Auth::user()->role == 1)
                        <li class="nav-item">
                            <a href="{{ route('admin.applications') }}" class="nav-link">Requests</a>
                        </li>
                    @elseif (Auth::user()->role == 3)
                        <li class="nav-item">
                            <a href="{{ route('create') }}" class="nav-link">New Art</a>
                        </li>
                    @endif
                @else
                    <li class="nav-item">
        ...
        <div class="row mt-3">
            ...
            <div class="col-md-4 mb-4">
                <div class="card">
                    ...
                    <!-- <img src="{{ asset($artwork->image) }}" class="card-img-top" alt="{{ $artwork->title }}"> -->
                    <div class="card-body">
                        <h5 class="card-title">{{ $artwork->title }}</h5>
                        <p class="card-text">Size: {{ $artwork->size }}</p>
                        <p class="card-text">Price: ₱{{ number_format($artwork->price, 2) }}</p>
                        <p class="card-text">Medium: {{ $artwork->medium }}</p>
                        <p><strong>Artist:</strong> {{ $artwork->user ? $artwork->user->name : 'Unknown' }}</p>
                        <!-- <a href="#" class="btn btn-outline-dark btn-lg text-uppercase fs-6 rounded-1">Buy</a> -->
                        <!-- Add to cart form -->
                        <!-- <form action="{{ route('addToCart', $artwork->id) }}" method="POST" style="display: inline-block;">
                            @csrf
                            <button type="submit" class="btn btn-outline-dark btn-lg text-uppercase fs-6 rounded-1">Buy</button>
                        </form> -->
                        <a href="{{ route('cart.add', $artwork->id) }}" class="btn btn-outline-dark btn-lg text-uppercase fs-6 rounded-1">
                          Buy
                        </a>
                    </div>
                </div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        addToCartForms.forEach(form => {
            form.addEventListener('submit', function (e) {
                ...
                const artworkTitle = form.querySelector('.card-title').textContent;
                const artworkPrice = form.querySelector('.card-text:nth-child(3)').textContent;
                addToCart(artworkId, artworkTitle, artworkPrice);
            });
        });

        function addToCart(artworkId, artworkTitle, artworkPrice) {
            // Append the selected artwork to the cart
            const cartItems = document.getElementById('cartItems');
            const listItem = document.createElement('li');
            listItem.className = 'list-group-item d-flex justify-content-between lh-sm';
            listItem.innerHTML = `
                <div>
                    <h6 class="my-0">${artworkTitle}</h6>
                    <small class="text-body-secondary">Quantity: 1</small>
                </div>
                <span class="text-body-secondary">${artworkPrice}</span>
            `;
            cartItems.appendChild(listItem);

            // Update the total price
            const totalPrice = document.getElementById('totalPrice');
            const currentTotalPrice = parseFloat(totalPrice.textContent.replace('₱', '').replace(',', ''));
            const artworkPriceValue = parseFloat(artworkPrice.replace('₱', '').replace(',', ''));
            const newTotalPrice = currentTotalPrice + artworkPriceValue;
